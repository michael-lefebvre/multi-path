...
const isFile = filePath => {
  try {
    if (fs.statSync(filePath)) return filePath;
  } catch (e) {
    return false;
  }
};
...
module.exports = function multipath(pathString) {
  try {
    ...
    if (!isDir && isFile(pathString)) return require(pathString);
  } catch (err) {
  ...
  // last check
  if (!isDir) throw new Error(`'${ pathString }' is not a directory or a file`);else {
    // if directory has an `index.js` file
    if (isFile(`${ pathString }/index.js`)) return require(`${ pathString }/index.js`);
  }
